...
The first and second latent variables are cross-validated.
...
################################# Females #####################################
    ...
    pval is (for lv = 1): 0.009900990099009901
    pval is (for lv = 2): 0.039603960396039604
    
################################## Males ######################################

    pval is (for lv = 0): 0.009900990099009901
    pval is (for lv = 1): 0.009900990099009901
    pval is (for lv = 2): 0.009900990099009901
...
sex_label = 'Male'   # Do you want to study PLS cross validation for female or male groups?
lv =  0              # You can change this to the lv of interest!

data = np.load(path_results + f'data_brain_array_{sex_label}_biobank.npy')
behaviour_data_array = np.load(path_results + f'data_bio_array_{sex_label}_biobank.npy')
...
plt.savefig(path_figures + 'UKBB_PLS_cross_validation_lv_' + str(lv) + f'_{sex_label}.svg',
        bbox_inches = 'tight',
        dpi = 300,
        transparent = True)
plt.show()
